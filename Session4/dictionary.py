...
vote_movie = data['results']
for vote_count in vote_movie:
    # print(movie['vote_count'])
    if vote_count['vote_count'] > 500:
        print(vote_count['original_title'])

input_genres = int(input()) # input genres and find movie
movies_list = data['results']
for movie in movies_list:
    if input_genres in movie['genres']:
        print(movie['original_title'])

for movie in movies_list:
    rl_date = movie['release_date']
    # if '2019' in rl_date:
    rl_year = rl_date.split('-')[0] #index
